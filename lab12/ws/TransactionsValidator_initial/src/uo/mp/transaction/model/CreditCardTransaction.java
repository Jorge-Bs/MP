package uo.mp.transaction.model;

import uo.mp.transaction.model.util.Lhun;

public class CreditCardTransaction extends Transaction {
	
	public static final String TYPE="cc";
	private String dueDate;
	private double maxAmount;

	public CreditCardTransaction(String date, String number, String dueDate,double maxAmount,
			double amount, String description) {
		
		super(date, number, amount, description);
		setDueDate(dueDate);
		setMaxAmount(maxAmount);
	}

	@Override
	public void validate() {
		validateDates(getDate(),getDueDate());
		validateOperation(getAmount(),getMaxAmount());
		validateNumber(getNumber());
	}

	
	private void validateNumber(String number) {
		if(!Lhun.isValid(number)){
			super.addFault("El numero de tarjeta: "+number+", es invalido");
		}
		
	}

	private void validateOperation(double amount, double maxAmount) {
		if(amount> maxAmount) {
			super.addFault("Sobre pasa el limite la operacion:"+amount+", el maximo permitido es:"+maxAmount);
		}
		
	}

	private void validateDates(String date, String dueDate) {
		if((date.compareTo(dueDate))>0) {
			super.addFault("La tarjeta está caducada");
		}
		
	}

	@Override
	public String serialize() {
		StringBuilder sb = new StringBuilder();
		sb.append(TYPE+";");
		sb.append(getDate()+";");
		sb.append(getNumber()+";");
		sb.append(getDueDate()+";");
		sb.append(getMaxAmount()+";");
		sb.append(getAmount()+";");
		sb.append(getDescription());
		return sb.toString();
	}

	private void setDueDate(String dueDate) {
		this.dueDate = dueDate;
	}

	private void setMaxAmount(double maxAmount) {
		this.maxAmount = maxAmount;
	}

	@Override
	public Transaction clone() {
		Transaction clone =  new CreditCardTransaction(getDate(), getNumber()
					, getDueDate(), getMaxAmount(), getAmount(),getDescription());
		clone.setFaults(getFaults());
		return clone;
	}

	private String getDueDate() {
		return dueDate;
	}

	private double getMaxAmount() {
		return maxAmount;
	}

	@Override
	public String toString() {
		StringBuilder builder = new StringBuilder();
		builder.append("CreditCardTransaction [date=");
		builder.append(getDate());
		builder.append(", number=");
		builder.append(getNumber());
		builder.append(", dueDate=");
		builder.append(getDueDate());
		builder.append(", maxAmount=");
		builder.append(getMaxAmount());
		builder.append(", amount=");
		builder.append(getAmount());
		builder.append(", description=");
		builder.append(getDescription());
		builder.append("]");
		if(hasFaults())
		{
			builder.append("\nfaults=");
			builder.append(getFaults());
		}
		return builder.toString();
	}
	
	public boolean equals(Object e) {
		if(e==this) {
			return true;
		}if(!(e instanceof CreditCardTransaction)) {
			return false;
		}else {
			Transaction et = (CreditCardTransaction) e;
			return (getNumber().equals(et.getNumber()) && getDate().equals(et.getDate()));
		}
	}
	
	
	
	

}
