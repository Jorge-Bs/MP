...
public class TransactionParser {
	...
	private void checkInside(Transaction object, List<Transaction> trans) throws RepeatedTransactionException {
		if(trans.contains(object)) {
			throw new RepeatedTransactionException("El fichero contiene dos transaciones iguales en linea: "+line);
		}
	...
	private Transaction parseLine(String line) throws InvalidLineFormatException{
		...
		default:
			throw new InvalidLineFormatException("Tipo de cuenta desconocido en linea: "+line);
		}
 	}
	
	private Transaction parseAcount(String[] list) throws InvalidLineFormatException {
	...
	private double toDouble(String dob) throws InvalidLineFormatException {
		try {
	...
	private Transaction parseCredit(String[] list) throws InvalidLineFormatException {
		ArgumentChecks.isTrue(list!=null, "Parametro incorrecto");
		checkLength(list,7);
	...
	private void checkLength(String[] list, int i) throws InvalidLineFormatException {
		if(!(list.length==i)) {
			throw new InvalidLineFormatException("La linea: "+line+" contine campos diferentes");
		}
