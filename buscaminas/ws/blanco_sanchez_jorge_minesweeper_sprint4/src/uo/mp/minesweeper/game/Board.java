...
public class Board {
	 ...
	 */
	private int calculateMines(double width, double height, double percentaje) {
		double value=width * height * percentaje / 100;
	...
	/**
	 * Establece la accion de la casilla
	 * 
	 */
	private void setAction() {
		for(int i=0;i< board.length;i++) {
			for(int j=0;j<board[i].length;j++) {
				...
				}else if(board[i][j].getValue()==Square.NOT_MINED) {
					board[i][j].setAction(new ClearAction(getNeighbours(i,j)));
				}
			}
		}
	}
	...
	/**
	 * Devuelve las casillas vecinas a una dada
	 * @param x posicion de la casilla en la fila
	 * @param y posicion de la casilla en la columna
	 */
	private List<Square> getNeighbours(int x,int y){
		...
		if(x==0) {
			if(y==0) {
				return getNeighboursAtCorner();
			}else if(y==board[x].length-1) {
				rotateBoard(3);
				List<Square> neighbour = getNeighboursAtCorner();
				rotateBoard(1);
				return neighbour;
			}else {
				return getNeighboursAtLine(y);
			}
		}
			...
			if(y==0) {
				...
				rotateBoard(2);
				List<Square> neighbour = getNeighboursAtLine(board[x].length-y-1);
				rotateBoard(2);
				return neighbour;
			}
		}else if(y==0) {
			rotateBoard(1);
			List<Square> neighbour = getNeighboursAtLine(board[0].length-x-1);
			rotateBoard(3);
			return neighbour;
		}else if(y==board[x].length-1) {
			rotateBoard(3);
			List<Square> neighbour = getNeighboursAtLine(x);
			rotateBoard(1);
			return neighbour;
		}else {
			return getNeighboursInside(x,y);
		}
	}
	
	/**
	 * Devuelve una lista con las casillas que hay en la esquina superior derecha
	 * @return neighbours de tipo List<Square>
	 */
	private List<Square> getNeighboursAtCorner(){
		List<Square> neighbour = new ArrayList<>();
		neighbour.add(board[0][1]);
		neighbour.add(board[1][1]);
		neighbour.add(board[1][0]);
		return neighbour;
		
	}
	
	/**
	 * Devuelve una lista con los vecino de las casillas que se situan en la fila superio
	 * 
	 * @param y posicion de la columna de la casilla
	 * @return neighbours de tipo List<Square>
	 */
	private List<Square> getNeighboursAtLine(int y){
		List<Square> neighbour = new ArrayList<>();
		neighbour.add(board[0][y-1]);
		neighbour.add(board[1][y-1]);
		neighbour.add(board[1][y]);
		neighbour.add(board[1][y+1]);
		neighbour.add(board[0][y+1]);
		return neighbour;
	}
	...
	/**
	 * Devuelve los vecinos de la casilla interna
	 * @param x fila
	 * @param y columna
	 */
	private List<Square> getNeighboursInside(int x,int y){
		List<Square> neighbour = new ArrayList<>();
		for(int i=y-1;i<=y+1;i++) {
			neighbour.add(board[x+1][i]);
			neighbour.add(board[x-1][i]);
		}
		neighbour.add(board[x][y-1]);
		neighbour.add(board[x][y+1]);
		return neighbour;
	}
...
//	}
	...
	public void flag(int x, int y) throws GameException {
		if (getFlag() > 0) {
			...
			throw new GameException("no quedan más banderas");
		}
	}
	 ...
	 * @throws GameException 
	 */
	public void unflag(int x, int y) throws GameException {
		board[x][y].unflag();
		setFlag(getFlag() + 1);
	...
	/**
	 * Hace stepOn sobre una casilla aleatoria con valor 0, es decir, una casilla que no contiene
	 * ni mina, ni valor numerico
	 * @throws GameException 
	 */
	public void uncoverWelcomeArea() throws GameException {
		Random rg = new Random();
		int x = rg.nextInt(getNumberOfRows());
		int y = rg.nextInt(getNumberOfColumns());
		if(board[x][y].getValue()!=Square.NOT_MINED || isACorner(x,y)) {
			uncoverWelcomeArea();
		}else {
			board[x][y].stepOn();
		}
	...
	/**
	 * Devuelve true si la casilla seleccionada esta en una esquina
	 * @param x
	 * @param y
