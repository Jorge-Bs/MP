...
import uo.mp.threads.sumas.sumador.SumadorSuperListo;
...
public class Main {
	...
	private static final int NUMERO_DE_SUMAS = 10000;
	private static final int NUMERO_SUMADORES = 1;
	...
	private void run() {
		...
		System.out.println("Fin: " + time + " milisegundos usando " + 
		NUMERO_SUMADORES + " sumadores para " + NUMERO_DE_SUMAS + " sumas");
	}
	...
	private List<Thread> construirSumadores(AlmacenSumas almacen) {
		...
		for (int i = 0; i< NUMERO_SUMADORES; i++){
			sumadores.add(new Thread(new SumadorPesao(almacen)));  //ESCOGER AQU� TIPO DE SUMADOR
			//sumadores.add(new Thread(new SumadorSuperListo(almacen)));
		}
		return sumadores;
	}
